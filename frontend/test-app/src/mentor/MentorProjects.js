...
const MentorProfile = () => {
  ...
  const [isProjectSelectedModalOpen, setProjectSelectedModalOpen] = useState(false);
  ...
  const handleProjectClick = (project) => {
    ...
    setSelectedProjectContent(`Selected project: ${project[1]}`);
    setProjectSelectedModalOpen(true);
    // Fetch students assigned to the selected project
    axios.post('http://127.0.0.1:5000/api/v1/mentor/projects/students', {
  ...
  return (
    <div className={styles.mainContainer}>
      ...
      <Modal isOpen={isAddProjectModalOpen} closeModal={() => setAddProjectModalOpen(false)}>
        {/* Content for the Add Project modal */}
      </Modal>
      <Modal isOpen={isProjectSelectedModalOpen} closeModal={() => setProjectSelectedModalOpen(false)}>
        {/* Content for the Selected Project modal */}
      </Modal>
      <ToastContainer/>
    </div>
  );
